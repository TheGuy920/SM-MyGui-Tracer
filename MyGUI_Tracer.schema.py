    ...
    for i in start.attrib:
        ...
        if not type(start.attrib[i]) == list:
            if i == "type":
                ...
                if not [ 0.0, 0.0, 0.0, 0.0 ] in final1[i]:
                    final1[i].append( [ 0.0, 0.0, 0.0, 0.0 ] )
        ...
        if not isKey:
            if len(value) > 0:
                ...
                if value in ["true", "false", "True", "False"]:
                    if not True in final[str(start.tag)][lastType][key]:
                        final[str(start.tag)][lastType][key].append(True)
                    ...
                    if re.search(r'^#(?:[0-9a-fA-F]{3}){1,2}$', value):
                        value = "Hex-Color"
                    value = "String"
                    if not value in final[str(start.tag)][lastType][key]:
                        final[str(start.tag)][lastType][key].append(value)
                    stringTable[lastType][key] = final[str(start.tag)][lastType][key]
    return final
